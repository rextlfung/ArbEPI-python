...
recon/operators_b0.py's mri_exp_approx wraps (mirtorch's own source raises
...
lowres_calib_recon_b0.py already validated) -- psi_recon is chosen so
that conjugating it reproduces that true inverse instead.

Does NOT reuse recon.operators_b0.build_encoding_operator_b0 directly, for
the same reason lowres_calib_recon_b0.py doesn't -- see that module's
...
like lowres_calib_recon_b0.py's `_build_calib_operator_b0` does.
...
def run_b0complex_corrected_calib_recon(
    ...
    """Core computation shared by main() and any other consumer wanting
...
if __name__ == '__main__':
    ...
    main(args.datdir, args.seqname, args.L, args.nbins, args.device, args.zero_pad_z)
