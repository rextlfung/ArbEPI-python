"""B0-corrected (and B0 + R2* complex-field) variants of recon/lowres_calib.py's
calibration-region reconstruction (.venv-recon). Subcommands:

    .venv-recon/bin/python -m recon.lowres_calib_b0 {b0,b0complex} ...
...
Usage (from repo root, .venv-recon):
    .venv-recon/bin/python -m recon.lowres_calib_b0 b0 <datdir> \
        [--seqname ArbEPI] [--device cuda]


Formerly recon/lowres_calib/lowres_calib_recon_b0complex.py
-----------------------------------------------------------
Generalized-complex-field-map variant of recon/lowres_calib_b0.py:
...
lowres_calib_b0.py already validated) -- psi_recon is chosen so
...
the same reason lowres_calib_b0.py doesn't -- see that module's
...
like lowres_calib_b0.py's `_build_calib_operator_b0` does.

Motivation (see CLAUDE.md's recon/ "B0 off-resonance correction" section
for the full derivation): lowres_calib_b0.py's phase-only correction
...
Usage (from repo root, .venv-recon):
    .venv-recon/bin/python -m recon.lowres_calib_b0 b0complex <datdir> \
        [--seqname ArbEPI] [--device cuda]
"""
...
def compute_calib_mask(omegas: np.ndarray) -> np.ndarray:
    """Same as recon/lowres_calib.py's -- duplicated, see module
...
def native_calib_grid(
    ...
    """Same as recon/lowres_calib.py's -- duplicated for the same
...
def _cli_b0() -> None:
    ...
    main_b0(args.datdir, args.seqname, args.L, args.nbins, args.device)
...
def nominal_te_s(scan_info_path: str, etl: int) -> float:
    ...
    incomplete subset of the full ETL (see lowres_calib_b0.py's
...
def _build_calib_operator_b0_complex(
    ...
    """Same construction as lowres_calib_b0.py's
...
def main_b0complex(
    ...
    result = run_b0complex_corrected_calib_recon(datdir, seqname, L, nbins, device, zero_pad_z)
    img_np, fov = result['img_np'], result['fov']
    grid, voxel_mm = result['grid'], result['voxel_mm']
    n_calib, calib_mask = result['n_calib'], result['calib_mask']
    Nx_eff, Ny_eff, Nz_eff = grid['Nx_eff'], grid['Ny_eff'], grid['Nz_eff']
...
def _cli_b0complex() -> None:
    ...
    parser.add_argument(
        ...
        help='zero-pad the deGRE-grid R2* estimate where the EPI z-FOV exceeds '
             "deGRE's fixed z-FOV, instead of raising (see docs/review-findings.md item 203)",
    )
    args = parser.parse_args()
    main_b0complex(args.datdir, args.seqname, args.L, args.nbins, args.device, args.zero_pad_z)


_COMMANDS = {
    'b0': _cli_b0,
    'b0complex': _cli_b0complex,
}


if __name__ == '__main__':
    if len(sys.argv) < 2 or sys.argv[1] not in _COMMANDS:
        sys.exit('usage: python -m recon.lowres_calib_b0 {' + ','.join(_COMMANDS) + '} ...')
    _cmd = sys.argv.pop(1)
